...
router.put("/:id", middleware.checkArticleOwnership, function(req, res) {
	Article.findByIdAndUpdate(req.params.id, {"title": req.body.article.title, "author.full_name": req.body.article.author.full_name, "image": req.body.article.image, "post": req.body.article.post}, function (err, articleUpdate) {
		if (err) {
			console.log(err);
		} else {	
...
router.post("/search", function(req, res) {
	Article.find({"$or": [{"title": {"$regex": req.body.search, "$options": "i"}}, {"author": {"$regex": req.body.search, "$options": "i"}}]}, function(err, matchingArticles) {
		if (err || matchingArticles.length < 1) {
			res.send("No matching results");
		} else {
			res.render("searchResults", {articles: matchingArticles, search: req.body.search});
		}
	});
});
